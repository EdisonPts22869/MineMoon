...
class User(Base):
    ...
    @property
    def vip_title(self):
        return {0: "Новичок", 1: "Бывалый шахтер", 2: "Старатель", 3: "Золотоискатель",
                4: "Хранитель недр", 5: "Властелин шахт"}.get(self.vip_level, "Легенда")
...
class Artifact(Base):
    ...
    def rarity_name(self):
        return {"common": "Обычный", "rare": "Редкий", "epic": "Эпический",
                "legendary": "Легендарный", "mythical": "Мифический"}.get(self.rarity, "Обычный")

    @property
    def type_name(self):
        return {"ring": "💍 Кольцо", "amulet": "📿 Амулет", "stone": "💎 Камень",
                "scroll": "📜 Свиток", "crystal": "🔮 Кристалл", "rune": "ᚱ Руна"}.get(self.artifact_type, "Артефакт")
...
ARENAS = [
    ...
    (10, "🏰 Каменоломня", "Следы цивилизации", 550, 275),
    (15, "👁 Oko бездны", "Бездонная шахта", 800, 400),
    (20, "🌟 Астральный разлом", "Редчайшие руды", 1050, 525),
    (25, "🔮 Хрустальный грот", "Магические кристаллы", 1300, 650),
    (30, "⛓ Цепи Тартара", "Врата в ад", 1550, 775),
    (40, "🏛 Храм забвения", "Руины богов", 2050, 1025),
    (50, "👑 Тронный зал", "Последняя шахта", 3000, 1500),
]

BOSSES = {}
for an, _, _, _, _ in ARENAS:
    BOSSES[an] = [
        (f"Хранитель {an}", "Страж", 500 + an * 80, 60 + an * 5, 15 + an // 3, 400 + an * 40, 150 + an * 20,
         f"Руда {an}", 0.4, None, 0, 0.05),
        (f"Старатель {an}", "Проклятый", 700 + an * 100, 80 + an * 6, 20 + an // 2, 600 + an * 50, 200 + an * 25,
         "Слиток", 0.35, None, 0, 0.06),
        (f"Владыка {an}", "Повелитель", 1000 + an * 150, 120 + an * 8, 30 + an * 2, 900 + an * 60, 300 + an * 35,
         "Самородок", 0.3, f"⭐ Ключ {an}", 0.05, 0.1),
    ]
BOSSES[1] = [
...
CASES = [
    {"name": "🎒 Снаряжение", "desc": "Базовый набор", "coins": 1000, "gems": 0, "vip": 0,
     "items": [("500 золотых", 0.5), ("Каска шахтера", 0.3), ("Кирка новичка", 0.2)]},
    {"name": "💎 Самоцветы", "desc": "Сундук", "coins": 0, "gems": 100, "vip": 1,
     "items": [("1000 золотых", 0.3), ("50 самоцветов", 0.3), ("Сапфировая кирка", 0.2), ("Рубиновая броня", 0.2)]},
    {"name": "👑 Старатель", "desc": "Для опытных", "coins": 0, "gems": 500, "vip": 3,
     "items": [("5000 золотых", 0.3), ("200 самоцветов", 0.2), ("Алмазная кирка", 0.2), ("Золотая корона", 0.15),
               ("Яйцо дракона", 0.15)]},
    {"name": "🌟 Легендарный", "desc": "Для Властелинов", "coins": 0, "gems": 1500, "vip": 5,
     "items": [("10000 золотых", 0.3), ("500 самоцветов", 0.2), ("Кирка титанов", 0.15), ("Древний дракон", 0.15),
               ("Титул Король шахт", 0.1), ("Артефакт недр", 0.1)]},
]

ARTIFACT_BAG = {"name": "🎒 Сумка", "coins": 5000, "gems": 100, "vip": 0, "count": 1,
                "w": {"common": 0.5, "rare": 0.3, "epic": 0.15, "legendary": 0.04, "mythical": 0.01}}
ARTIFACT_SUITCASE = {"name": "🧳 Чемодан", "coins": 0, "gems": 500, "vip": 2, "count": 3,
                     "w": {"common": 0.2, "rare": 0.35, "epic": 0.25, "legendary": 0.15, "mythical": 0.05}}
ARTIFACT_ELITE = {"name": "👑 Элитный", "coins": 0, "gems": 2000, "vip": 5, "count": 5,
                  "w": {"common": 0.05, "rare": 0.15, "epic": 0.25, "legendary": 0.35, "mythical": 0.2}}
        ...
        if ar:
            for i, bd in enumerate(bs, 1):
                if not s.query(Boss).filter(Boss.arena_id == ar.id, Boss.boss_number == i).first():
                    s.add(Boss(
                        arena_id=ar.id, boss_number=i, name=bd[0], title=bd[1],
                        health=bd[2], damage=bd[3], defense=bd[4],
                        reward_coins=bd[5], reward_exp=bd[6],
                        drop_material=bd[7], drop_chance=bd[8],
                        artifact_drop_chance=bd[11] if len(bd) > 11 else 0.1
                    ))
    for c in CASES:
        if not s.query(Case).filter(Case.name == c["name"]).first():
            s.add(Case(name=c["name"], description=c["desc"], price_coins=c["coins"],
                       price_gems=c["gems"], required_vip=c["vip"], drop_table=json.dumps(c["items"])))
    s.commit()
    s.close()
    print(f"✅ БД готова: {len(ARENAS)} шахт, {sum(len(b) for b in BOSSES.values())} боссов")
...
def get_user(session, tg_id, create=True):
    ...
    if not u and create:
        ...
        session.add(u)
        session.commit()
        for it in [("weapon", "Кирка новичка", 1, True), ("armor", "Роба шахтера", 1, True),
                   ("material", "Пайка шахтера", 5, False)]:
...
def update_stats(u):
    ...
    for a in s.query(Artifact).filter(Artifact.owner_id == u.id, Artifact.equipped_slot > 0).all():
        ...
        bdf += a.defense_bonus
        bh += a.health_bonus
        cc += a.crit_chance_bonus
        cd += a.crit_damage_bonus
        dc += a.dodge_bonus
        ls += a.lifesteal_bonus
    s.close()
    if u.is_vip:
        ...
        bdf = int(bdf * m)
        bh = int(bh * m)
        cc += 0.05 + u.vip_level * 0.03
        cd += 0.2 + u.vip_level * 0.2
        dc += 0.05 + u.vip_level * 0.02
        ls += 0.05 * u.vip_level
    u.damage = bd
    u.defense = bdf
    u.max_health = bh
    u.critical_chance = min(cc, 0.8)
    u.critical_damage = min(cd, 5.0)
    u.dodge_chance = min(dc, 0.5)
    u.lifesteal = min(ls, 0.5)
    u.max_energy = 100 + (u.lantern_level - 1) * 20


def create_artifact(oid, weights=None):
    if not weights:
        weights = {"common": 0.5, "rare": 0.3, "epic": 0.13, "legendary": 0.05, "mythical": 0.02}
    ...
    s = SessionLocal()
    art = Artifact(
        owner_id=oid, name=tp["name"], artifact_type=t, rarity=r,
        damage_bonus=tp.get("damage_bonus", 0), defense_bonus=tp.get("defense_bonus", 0),
        health_bonus=tp.get("health_bonus", 0), crit_chance_bonus=tp.get("crit_chance_bonus", 0.0),
        crit_damage_bonus=tp.get("crit_damage_bonus", 0.0), dodge_bonus=tp.get("dodge_bonus", 0.0),
        lifesteal_bonus=tp.get("lifesteal_bonus", 0.0), exp_bonus=tp.get("exp_bonus", 0.0),
        coin_bonus=tp.get("coin_bonus", 0.0)
    )
    s.add(art)
    s.commit()
    s.refresh(art)
    # Copy id before closing session to avoid DetachedInstanceError
    art_id = art.id
    art_name = art.name
    art_rarity = art.rarity
    art_type = art.artifact_type
    s.close()
    # Return a simple namespace to avoid accessing detached ORM object
    class _Art:
        pass
    result = _Art()
    result.id = art_id
    result.name = art_name
    result.rarity = art_rarity
    result.artifact_type = art_type
    result.rarity_color = {"common": "⬜", "rare": "🟦", "epic": "🟪", "legendary": "🟨", "mythical": "🟥"}.get(art_rarity, "⬜")
    return result
...
def main_menu():
    ...
    for t, c in [
        ("🏟 Шахты", "m_arenas"), ("👤 Профиль", "m_profile"),
        ("🎒 Инвентарь", "m_inv"), ("💍 Артефакты", "m_arts"),
        ("⚔️ Кузница", "m_upgrade"), ("📦 Кейсы", "m_cases"),
        ("🎒 Сумка артефактов", "m_bag"), ("🧳 Чемодан артефактов", "m_case"),
        ("👑 Элитный кейс", "m_elite"), ("🎁 Бонус", "m_daily"),
        ("🔄 Перерождение", "m_rebirth"), ("🏆 Топ", "m_top"),
        ("🎟 Промокод", "m_promo"), ("💎 Магазин", "m_donate"),
        ("❓ Помощь", "m_help"),
    ]:
...
def admin_kb():
    b = InlineKeyboardBuilder()
    for t, c in [
        ("📢 Рассылка", "a_broadcast"), ("👤 Список игроков", "a_find"),
        ("💰 Выдать золото", "a_give_coins"), ("💎 Выдать самоцветы", "a_give_gems"),
        ("👑 Выдать VIP", "a_set_vip"), ("🔨 Забанить", "a_ban"),
        ("🔓 Разбанить", "a_unban"), ("💍 Создать артефакт", "a_artifact"),
        ("🎟 Создать промокод", "a_create_promo"), ("📋 Список промокодов", "a_list_promo"),
        ("🗑 Удалить промокод", "a_delete_promo"), ("📊 Статистика", "a_stats"),
        ("◀ Назад", "m_main"),
    ]:
        b.button(text=t, callback_data=c)
    b.adjust(1)
...
async def start(msg: Message):
    ...
    s.commit()
    s.close()
    await msg.answer("⛏ <b>ТЕМНЫЕ ШАХТЫ</b>\n/menu - меню\n/profile - профиль\n/admin - админка",
...
async def profile(msg: Message):
    ...
    s.commit()
    await msg.answer(
        f"⛏ {u.first_name or u.username}\n"
        f"🏅 {u.vip_title} ур.{u.vip_level}\n"
        f"⚔️ Урон: {u.damage} | 🛡 Защита: {u.defense}\n"
        f"❤️ HP: {u.health}/{u.max_health}\n"
        f"💰 Золото: {u.coins} | 💎 Самоцветы: {u.gems}\n"
        f"🏟 Шахта: {u.arena}/50"
    )
    s.close()
...
async def cb_arts(cb: CallbackQuery):
    ...
    txt = ("💍 Артефакты:\n" +
           "\n".join(f"{a.rarity_color} {a.name} ур.{a.level} [{'✅' if a.equipped_slot else '❌'}]" for a in arts)
           if arts else "Нет артефактов")
    b = InlineKeyboardBuilder()
    for a in arts[:10]:
        b.button(text=f"{a.rarity_color} {a.name}", callback_data=f"art_{a.id}")
    b.button(text="◀ Назад", callback_data="m_main")
    b.adjust(1)
    s.close()
...
async def cb_art_info(cb: CallbackQuery):
    ...
    s = SessionLocal()
    a = s.query(Artifact).filter(Artifact.id == aid).first()
    if not a:
        await cb.answer("Не найден!")
        s.close()
        return
    txt = (f"{a.rarity_color} {a.name} ({a.rarity_name})\n"
           f"{a.type_name} ур.{a.level}\n"
           f"⚔️ +{a.damage_bonus} | 🛡 +{a.defense_bonus} | ❤️ +{a.health_bonus}")
    b = InlineKeyboardBuilder()
    if a.equipped_slot:
        b.button(text="🔽 Снять", callback_data=f"uneq_{a.id}")
    else:
        b.button(text="⚡ Экипировать", callback_data=f"eq_{a.id}")
    b.button(text="🗑 Утилизировать", callback_data=f"disp_{a.id}")
    b.button(text="◀ Назад", callback_data="m_arts")
    b.adjust(2)
    s.close()
    await cb.message.edit_text(txt, reply_markup=b.as_markup())
    await cb.answer()
...
async def cb_equip(cb: CallbackQuery):
    ...
    if not free:
        await cb.answer("Нет свободных слотов!")
        s.close()
        return
    a.equipped_slot = free[0]
    s.commit()
    s.close()
    await cb.answer("Экипирован!")
    await cb_arts(cb)
...
async def cb_upgrade(cb: CallbackQuery):
    s = SessionLocal()
    u = get_user(s, cb.from_user.id)
    pc = 1000 * u.pickaxe_level
    ac = 800 * u.armor_level
    txt = (f"⚔️ Кузница\n💰 {u.coins} золота\n\n"
           f"⛏ Кирка ур.{u.pickaxe_level} - {pc}💰\n"
           f"🛡 Броня ур.{u.armor_level} - {ac}💰")
    b = InlineKeyboardBuilder()
    b.button(text="⛏ Кирка", callback_data="up_pick")
    b.button(text="🛡 Броня", callback_data="up_armor")
    b.button(text="◀ Назад", callback_data="m_main")
    b.adjust(2)
    s.close()
    await cb.message.edit_text(txt, reply_markup=b.as_markup())
...
async def cb_open_case(cb: CallbackQuery):
    ...
    s = SessionLocal()
    u = get_user(s, cb.from_user.id)
    c = s.query(Case).filter(Case.id == cid).first()
    if not c:
        await cb.answer("Не найден!")
        s.close()
        return
    if c.price_coins > 0 and u.coins < c.price_coins:
        await cb.answer("Мало💰!")
        s.close()
        return
    if c.price_gems > 0 and u.gems < c.price_gems:
        await cb.answer("Мало💎!")
        s.close()
        return
    u.coins -= c.price_coins
    u.gems -= c.price_gems
    ...
    s.commit()
    s.close()
    await cb.message.edit_text(f"🎉 {reward}!", reply_markup=main_menu())
    await cb.answer()
...
async def cb_case(cb: CallbackQuery):
    s = SessionLocal()
    u = get_user(s, cb.from_user.id)
    if u.vip_level < ARTIFACT_SUITCASE["vip"]:
        await cb.answer("VIP 2+!")
        s.close()
        return
    if u.gems < ARTIFACT_SUITCASE["gems"]:
        await cb.answer("Мало💎!")
        s.close()
        return
...
async def cb_elite(cb: CallbackQuery):
    s = SessionLocal()
    u = get_user(s, cb.from_user.id)
    if u.vip_level < ARTIFACT_ELITE["vip"]:
        await cb.answer("VIP 5!")
        s.close()
        return
    if u.gems < ARTIFACT_ELITE["gems"]:
        await cb.answer("Мало💎!")
        s.close()
        return
...
async def cb_daily(cb: CallbackQuery):
    ...
    u = get_user(s, cb.from_user.id)
    now = datetime.now()
    if u.last_daily and u.last_daily.date() == now.date():
        await cb.answer("Уже получен!")
        s.close()
        return
    if u.last_daily and (now - u.last_daily).days > 1:
        u.daily_streak = 0
    u.daily_streak += 1
    u.last_daily = now
    bonus = min(u.daily_streak, 7)
    coins = 500 * bonus
    gems = 25 * bonus
    if u.is_vip:
        coins = int(coins * 1.5)
    ...
    s.commit()
    s.close()
    await cb.message.edit_text(f"🎁 +{coins}💰 +{gems}💎\nСерия: {u.daily_streak}/7", reply_markup=main_menu())
    await cb.answer()
...
async def cb_rebirth(cb: CallbackQuery):
    ...
    u = get_user(s, cb.from_user.id)
    if u.arena < 50:
        await cb.answer("Нужна 50 шахта!")
        s.close()
        return
    u.arena = 1
    u.pickaxe_level = 1
...
async def promo_use(msg: Message, state: FSMContext):
    ...
    if s.query(PromoUsed).filter(PromoUsed.user_id == u.id, PromoUsed.promo_id == p.id).first():
        await msg.answer("❌ Уже использован!")
        await state.clear()
        s.close()
        return
    u.coins += p.reward_coins
    u.gems += p.reward_gems
    if p.reward_item:
        s.add(Inventory(user_id=u.id, item_type="special", item_name=p.reward_item, quantity=1))
    s.add(PromoUsed(user_id=u.id, promo_id=p.id))
    p.uses_count += 1
    s.commit()
    s.close()
    await msg.answer("✅ Активирован!", reply_markup=main_menu())
    await state.clear()
...
async def broadcast(msg: Message, state: FSMContext):
    if not is_admin(msg.from_user.id):
        return
    s = SessionLocal()
    users = s.query(User).filter(User.notifications_enabled == True).all()
    ok = 0
    for u in users:
        try:
            await bot.send_message(u.user_id, f"📢 {msg.text}")
            ok += 1
    ...
    s.close()
    await state.clear()
    await msg.answer(f"✅ Отправлено {ok} из {len(users)}", reply_markup=admin_kb())
...
async def cb_afind(cb: CallbackQuery):
    ...
    for u in users:
        txt += f"ID: <code>{u.user_id}</code> - {u.first_name or 'Нет'}\n💰{u.coins} 💎{u.gems} 🏟{u.arena}\n\n"
    s.close()
...
async def cb_acoins(cb: CallbackQuery, state: FSMContext):
    if not is_admin(cb.from_user.id):
        await cb.answer("⛔")
        return
    await cb.message.edit_text("💰 Введите ID и сумму: <code>123456789 10000</code>")
    await state.set_state(AdminStates.waiting_for_give_coins)
    await cb.answer()
...
async def cb_agems(cb: CallbackQuery, state: FSMContext):
    if not is_admin(cb.from_user.id):
        await cb.answer("⛔")
        return
    await cb.message.edit_text("💎 Введите ID и сумму: <code>123456789 500</code>")
    await state.set_state(AdminStates.waiting_for_give_gems)
    await cb.answer()
...
async def cb_avip(cb: CallbackQuery, state: FSMContext):
    if not is_admin(cb.from_user.id):
        await cb.answer("⛔")
        return
    await cb.message.edit_text("👑 Введите ID и уровень (1-5): <code>123456789 3</code>")
    await state.set_state(AdminStates.waiting_for_set_vip)
    await cb.answer()
...
async def set_vip(msg: Message, state: FSMContext):
    ...
    try:
        ...
        await msg.answer("❌ Формат: ID уровень")
    await state.clear()
...
async def cb_aban(cb: CallbackQuery, state: FSMContext):
    if not is_admin(cb.from_user.id):
        await cb.answer("⛔")
        return
    await cb.message.edit_text("🔨 Введите ID и причину: <code>123456789 спам</code>")
...
async def cb_astats(cb: CallbackQuery):
    ...
    s = SessionLocal()
    txt = (f"📊 Статистика:\n"
           f"👥 Игроков: {s.query(User).count()}\n"
           f"🔨 Забанено: {s.query(User).filter(User.is_banned == True).count()}\n"
           f"👑 VIP: {s.query(User).filter(User.vip_level > 0).count()}\n"
           f"💍 Артефактов: {s.query(Artifact).count()}\n"
           f"🎟 Промокодов: {s.query(PromoCode).count()}")
    s.close()
    await cb.message.edit_text(txt, reply_markup=admin_kb())
    await cb.answer()
...
async def cb_list_promo(cb: CallbackQuery):
    ...
    if not promos:
        ...
        for p in promos:
            ...
            exp_text = ""
            if p.expires_at and not p.is_permanent:
                exp_text = f" | До: {p.expires_at.strftime('%d.%m.%Y')}"
            txt += f"{pt}{st} <code>{p.code}</code>\n├ 💰{p.reward_coins} 💎{p.reward_gems}"
            if p.reward_item:
                txt += f"\n├ 🎁 {p.reward_item}"
            txt += f"\n└ 👥 {p.uses_count}/{p.uses_limit}{exp_text}\n\n"
    s.close()
    await cb.message.edit_text(txt, reply_markup=admin_kb())
    await cb.answer()
...
async def cb_delete_promo_menu(cb: CallbackQuery):
    ...
    if not promos:
        await cb.answer("Нет промокодов!")
        s.close()
        return
    b = InlineKeyboardBuilder()
    for p in promos[:20]:
        b.button(text=f"🗑 {p.code}", callback_data=f"delpromo_{p.id}")
    b.button(text="◀ Назад", callback_data="admin_panel")
    b.adjust(2)
    s.close()
    await cb.message.edit_text("Выберите для удаления:", reply_markup=b.as_markup())
    await cb.answer()
...
async def cb_delete_promo_confirm(cb: CallbackQuery):
    ...
    s.close()
    await cb.message.edit_text("✅ Промокод удалён!", reply_markup=admin_kb())
    # Removed duplicate cb.answer() call that caused "query is too old" error
...
async def cb_aartifact(cb: CallbackQuery, state: FSMContext):
    if not is_admin(cb.from_user.id):
        await cb.answer("⛔")
        return
    await cb.message.edit_text("💍 Введите ID игрока:")
    await state.set_state(AdminStates.waiting_for_artifact_target)
    await cb.answer()
...
async def a_type(cb: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(AdminStates.waiting_for_artifact_rarity)
    await cb.answer()
...
async def a_rarity(cb: CallbackQuery, state: FSMContext):
    ...
    r = cb.data.split("_")[1]
    s = SessionLocal()
    target = get_user(s, tid)
    av = [a for a in ARTIFACTS[at] if a["rarity"] == r] or ARTIFACTS[at]
    tp = random.choice(av)
    art = Artifact(
        owner_id=target.id, name=tp["name"], artifact_type=at, rarity=r,
        damage_bonus=tp.get("damage_bonus", 0), defense_bonus=tp.get("defense_bonus", 0),
        health_bonus=tp.get("health_bonus", 0)
    )
    s.add(art)
    s.commit()
    art_name = art.name
    s.close()
    await cb.message.edit_text(f"✅ Артефакт {art_name} выдан!", reply_markup=admin_kb())
    try:
        await bot.send_message(tid, f"🎁 Админ выдал: {art_name}!")
    except:
        pass
    await state.clear()
    await cb.answer()
...
async def cb_create_promo_start(cb: CallbackQuery, state: FSMContext):
    if not is_admin(cb.from_user.id):
        await cb.answer("⛔")
        return
    await cb.message.edit_text("🎟 Введите КОД промокода:")
...
async def promo_code_input(msg: Message, state: FSMContext):
    ...
    code = msg.text.upper()
    s = SessionLocal()
    if s.query(PromoCode).filter(PromoCode.code == code).first():
        await msg.answer("❌ Уже существует!")
        s.close()
        return
    s.close()
    await state.update_data(promo_code=code)
    await msg.answer("💰 Введите монеты:")
    await state.set_state(AdminStates.waiting_for_promo_coins)
...
async def promo_coins_input(msg: Message, state: FSMContext):
    ...
    await msg.answer("💎 Введите самоцветы:")
    await state.set_state(AdminStates.waiting_for_promo_gems)
...
async def promo_gems_input(msg: Message, state: FSMContext):
    ...
    await msg.answer("👥 Лимит использований:")
...
async def promo_limit_input(msg: Message, state: FSMContext):
    ...
    await msg.answer("⏰ Дней или 'permanent':")
...
async def promo_duration_input(msg: Message, state: FSMContext):
    ...
    await msg.answer("🎁 Предмет или '-':")
    await state.set_state(AdminStates.waiting_for_promo_item)
...
async def promo_item_input(msg: Message, state: FSMContext):
    ...
    s.add(PromoCode(
        code=data["promo_code"], reward_coins=data["promo_coins"],
        reward_gems=data["promo_gems"], uses_limit=data["promo_limit"],
        expires_at=exp, is_permanent=data["promo_permanent"], reward_item=item
    ))
    s.commit()
    s.close()
...
async def cb_arenas(cb: CallbackQuery):
    ...
    b.button(text="◀", callback_data="m_main")
    b.adjust(5)
    s.close()
    await cb.message.edit_text(f"⛏ Шахты (текущая: {u.arena}/50)", reply_markup=b.as_markup())
    await cb.answer()
...
async def cb_arena(cb: CallbackQuery):
    ...
    b = InlineKeyboardBuilder()
    for boss in bs:
        b.button(text=f"⚔️ {boss.name}", callback_data=f"bs_{an}_{boss.boss_number}")
    b.button(text="◀", callback_data="m_arenas")
    b.adjust(1)
    s.close()
    await cb.message.edit_text(f"⛏ {ar.name}\n{ar.description}\n💰 {ar.reward_coins}", reply_markup=b.as_markup())
    await cb.answer()
...
async def cb_fight(cb: CallbackQuery, state: FSMContext):
    ...
    b.button(text="🏃 Сбежать", callback_data="m_arenas")
    b.adjust(2)
    await cb.message.edit_text(
        f"⚔️ БОЙ!\n👹 {boss.name}\n❤️ {boss.health}\n\nТы: ❤️ {u.max_health} | ⚔️ {u.damage}",
        reply_markup=b.as_markup()
    )
    await cb.answer()
...
async def cb_potion(cb: CallbackQuery):
    ...
    if not pot or pot.quantity < 1:
        await cb.answer("Нет зелий!")
        s.close()
        return
    pot.quantity -= 1
    if pot.quantity <= 0:
        s.delete(pot)
    heal = 50
    f["uhp"] = min(f["uhp"] + heal, u.max_health)
    s.commit()
    s.close()
...
async def cb_attack(cb: CallbackQuery, state: FSMContext):
    ...
    if random.random() < u.critical_chance:
        dmg = int(dmg * u.critical_damage)
    f["bhp"] -= dmg
    if f["bhp"] <= 0:
        u.coins += boss.reward_coins
        u.exp += boss.reward_exp
        u.gems += boss.reward_gems
        txt = f"🏆 Победа!\n💰 +{boss.reward_coins}"
        if random.random() < boss.artifact_drop_chance:
            art = create_artifact(u.id)
            txt += f"\n💍 +{art.rarity_color} {art.name}!"
        if an == u.arena and an < 50:
            u.arena += 1
            txt += f"\n⛏ Шахта {u.arena} открыта!"
        s.commit()
        s.close()
        del active_fights[cb.from_user.id]
        await state.clear()
        await cb.message.edit_text(txt, reply_markup=main_menu())
